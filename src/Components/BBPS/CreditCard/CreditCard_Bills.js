...
const CreditCard_Bills = () => {
  ...
  useEffect(() => {
    fetch(`${API_IP_ADDRESS}/bills/getCreditCardBillers`, {
      method: "GET",
      headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`${API_IP_ADDRESS}/bills/CreditCardBillfetchRequest`, {
        method: "POST",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
  ...
  const handlePaynow = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_IP_ADDRESS}/bills/CreditCardbillPayment`,
        {
          method: "POST",
          headers: {
            Accept: "application/json",
                    ...
                    <div>
                      <div style={{ display: "flex" }}>
                        <div style={{ width: "50%" }}>
                          ...
                          <p>Mobile Number:{formData['Registered Mobile Number']}</p>
                          <p>Bill Amount:{biller_details.additionalParams.MinimumDueAmount}</p>
                        </div>
                        <div style={{ width: "50%" }}>
                          <p>Bill Period:{biller_details.billPeriod}</p>
                          <p>Due Date:{biller_details.dueDate}</p>
                          <p>Bill Date:{biller_details.billDate}</p>
                          <p>Timestamp:{biller_details.requestTimeStamp}</p>

                        </div>
                      </div>
                      <p>Reference Id: {biller_details.refId}</p>
                      <p>ApproveRefId:{biller_details.approvalRefNum}</p>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', height: '100%', width: "100%" }}>
                    ...
                    <div
                      ...
                      <p style={{ color: 'black', fontWeight: 'bold' }}>
                        <h4>Your  CreditCard  Bill Paid Successfully</h4>
                      </p>
                    </div>
                    ...
                    <div
                      ...
                      <Typography>
                        <div>
                          <p style={{ fontSize: "12px", textAlign: "center" }}>Your  CreditCard Bill Paid Successfully</p>
                          <div style={{ display: "flex", fontSize: "12px", justifyContent: "space-between" }}>
                            <div>
                              ...
                              <p>Amount:{payment_details.result.payload.paidAmount}</p>
                            </div>
                            <div>
                              <p>Bill Period: {biller_details.billPeriod}</p>
                              <p>Due Date: {biller_details.dueDate}</p>
...
export default CreditCard_Bills;